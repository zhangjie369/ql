 ...
 * export hqcsh='accountId&accountId'
 */
const $ = new Env('好奇车生活')
//const Cheryfs = ($.isNode() ? JSON.parse(process.env.Cheryfs) : $.getjson("Cheryfs")) || [];
const Cheryfss = process.env.hqcsh;
const CheryfsList = Cheryfss.split('&');
const Cheryfs = [];
for (const item of CheryfsList) {
        let items = {'accountId':item}
        Cheryfs.push(items)
    }
let accountId = ''
let notice = ''
...
async function main() {
    ...
    for (const item of Cheryfs) {
        try {
            ...
            console.log("————————————");
            console.log("每日任务");
            for (const taskItem of taskItemIdArr) {
                let taskItemId = Object.keys(taskItem)[0];
                try {
                    let taskItemAchieveDetail = await commonGet(`/task/taskItemAchieveDetail?taskItemId=${taskItemId}`);
                    
                    // 如果返回数据为空或无效，跳过当前任务
                    if (!taskItemAchieveDetail || !taskItemAchieveDetail.result) {
                        console.log(`任务ID ${taskItemId} 返回数据为空或无效，跳过此任务`);
                        break; // 跳出任务遍历，进入查询积分步骤
                    }
                    ...
                    console.log(`处理任务ID ${taskItemId} 出错，跳过此任务`);
                    console.error(error);
                    break; // 如果出现错误，跳出任务遍历，进入查询积分步骤
                }
            }
            ...
            console.log(`用户 ${item.accountId} 的任务出错，跳过此用户`);
            console.error(error);
        }
    }
    if (notice) {
